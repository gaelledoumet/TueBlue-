		  ...
		  $("#addtocart").click(function()
			{
				...
				$.ajax({
						...
						success: function(data){
							if((data=="Please choose the convenient color.")||(data=="Please choose the convenient size."))
							{
			...
			{
				var pcode=$(this).parent().parent().parent().children().eq(2).html();
		...
		<!-- Modal -->
			...
			<div class="twoColumns container pt-xl-23 pb-xl-20 pt-lg-20 pb-lg-20 py-md-16 py-10">
				<div class="row mb-6">
					...
					$lastname=$row10[1];
							?>
						<div class="wow bounceInUp productSliderImage mb-lg-0 mb-4">
							<div>
					...
					<div class="col-12 col-lg-6 order-lg-3">
						...
						<div class="productTextHolder overflow-hidden">
							...
							<div class="holder overflow-hidden d-flex flex-wrap mb-6">
								<input id="quantity" type="number" autocomplete="off" onkeydown="return false" placeholder="1" min="1" max=<?=$row[4]?>/>
								<?php if (isset($_SESSION["signedin"])){?>
									<a id="addtocart" class="btn btnTheme btnShop fwEbold text-white md-round py-3 px-4 py-md-3 px-md-4">Add To Cart <i class="fas fa-arrow-right ml-2"></i></a>
								<?php } else{ ?>
									<a id="addtosession" class="btn btnTheme btnShop fwEbold text-white md-round py-3 px-4 py-md-3 px-md-4 ">Add To Cart <i class="fas fa-arrow-right ml-2"></i></a>
									<?php } ?>
			...
			<?php 		
						...
						$stmt2=$mysqli->prepare("SELECT pcode,pname, image_url, oldprice, newprice, quantity FROM products WHERE sellers_username = ?");
						$stmt2->bind_param("s",$name);
						$stmt2->execute();
						$stmt2->store_result();
						$stmt2->bind_result($product_code,$pname,$url,$oldprice,$newprice,$quantity);
						$count_sellers_products=$stmt2->num_rows;
						if ($count_sellers_products>1)
						{?>
			<section class="wow slideInLeft featureSec container overflow-hidden pt-xl-12 pb-xl-29 pt-lg-10 pb-lg-14 pt-md-8 pb-md-10 py-5">
				<div class="row">
					<!-- mainHeader -->
					<header class="col-12 mainHeader mb-5 text-center">
						...
						<p>Like this product? Check the same seller's supply</p>
					</header>
				</div>
				<div class="row">
					<?php 
					$count=0;	
					 	while(($stmt2->fetch())&&($count<4)) {
					...
					<div class="col-12 col-sm-6 col-lg-3 featureCol position-relative mb-7">
						<div class="border">
							<div class="imgHolder position-relative w-100 overflow-hidden">
								...
								<span style="visibility:hidden"><?=$product_code?></span>
							</div>
							<div class="text-center py-5 px-4">
								...
								<?php } 
									if($quantity == 0){?>
										  <span class="hotOffer fwEbold text-uppercase text-white position-absolute d-block" style="right: 190px; font-size: 15px; margin-top:22px">Sold out</span>
									<?php } ?>
							</div>
						</div>
					</div>
