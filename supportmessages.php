 ...
 <script>
	 ...
	 $(document).ready(function($){
			...
			$(".remove_message").click(function(){
				 id=$(this).parent().children().eq(5).children().eq(0).html();
				$(".modal-body").html("Are you sure you want to remove message "+ id +"?");
				$("#modalbutton").click();
			});

			$("#confirm").click(function(){
				$.ajax({ url: 'removemessage.php',
                    type: 'POST',
                    data:{
							"id":id
                    },
	...
	<main style="background-image: url(images/blackwallpaper.jpg);">
      ...
      <section class="featureSec container-fluid overflow-hidden pt-xl-12 pt-lg-10 pt-md-80 pt-5 pb-xl-10 pb-lg-4 pb-md-2 px-xl-14 px-lg-7">
        ...
        <header class="col-12 mainHeader mb-7 text-center">
                    <h2 style="color:lightgrey" class="headingIV playfair fwEblod mb-4">Help and support</h2> 
					...
					$results = mysqli_query($mysqli, "SELECT name,email,message,phone_number,hs_id FROM help_and_support");
					$i=0;
					...
					?>
						<div class="wow slideInLeft col-12 pl-xl-23 mb-lg-6 mb-10">
							<div class="stepCol position-relative bg-lightGray py-6 px-6">
								<a class=" remove_message fas fa-times float-right"></a>
								<strong class="mainTitle text-uppercase mt-n8 mb-5 d-block text-center py-1 px-3"> <?=$row[0]?></strong>
                                <h2 class="headingV fwEblod mb-3">Email: <?=$row[1]?></h2>
                                <h3 class="headingV fwEblod mb-3">Tel: <?=$row[3]?></h3>
								<p class="mb-5"><?=$row[2]?> </p>
								<p>Message ID: <span><?=$row[4]?></span></p>
							</div>
						</div>
						<br>
						<?php } else{ ?>
