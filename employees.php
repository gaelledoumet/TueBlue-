...
<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
	...
	<script>
      ...
      {
		  var employee_to_fire;
          $(".fire").click(function()
            {
				employee_to_fire=$(this).parent().parent().children().eq(1).children().eq(0).html();
				$(".modal-body").html("Are you sure you want to fire "+ employee_to_fire+"?");
				$("#modalbutton").click();

		  });
		  
		  $("#confirm").click(function(){
			$.ajax({ url: 'deleteemployee.php',
                    type: 'POST',
                    data:{
                          "email":employee_to_fire,
                    },
                    dataType: 'text',
                          success: function(txt) {
                              if(txt.indexOf("successfully deleted")>-1)
                                {
  ...
  <body>
								...
								<nav class="navbar navbar-expand-lg navbar-light p-0 pageNav2 position-static">
									...
									<div class="collapse navbar-collapse" id="navbarNav">
										<ul class="navbar-nav mx-auto text-uppercase d-inline-block">
											...
											<li class="nav-item dropdown">
												<a class="d-block" role="button" aria-haspopup="true" aria-expanded="false" href="addemployee.html">Add Employee</a>
											</li>
											<li class="nav-item dropdown">
	...
	<h2 class="headingIV playfair fwEblod mb-4">Employees</h2> 
    ...
    $stmt2=$mysqli->prepare("SELECT employees.fullname, employees.email, employees.phonenumber, employees.address FROM employees");
    $stmt2->execute();
    $stmt2->store_result();
