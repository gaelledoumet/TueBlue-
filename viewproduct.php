...
$(document).ready(function($){
	var pcode;
		...
		$("#bt_delete").click(function(){
			pcode=$("#code").html();
			$(".modal-body").html("Are you sure you want to remove this product?");
			$("#modalbutton").click();
		});

		$("#confirm").click(function(){
			$.ajax({
					url: 'removesupply.php',
					type: "POST",
					datatype: "text",
					data: { "code":pcode, },
						...
						<div class="productTextHolder overflow-hidden">
							...
							$results2 = mysqli_query($mysqli, "SELECT rating FROM rates WHERE products_pcode={$store}");
							  ...
							  if($sumofrates == 0)
							  {
								...
								<li class="mr-2"><a href="javascript:void(0);"><i class="far fa-star"></i></a></li>
								<li class="mr-2"><a href="javascript:void(0);"><i class="far fa-star"></i></a></li>
								<li>( <?=$sumofrates?> customer reviews )</li>
							    </ul>
								   <?php
							  }
							  else{
							...
							<p class="mb-5"><?=$row[5]?></p>
							<ul class="list-unstyled productInfoDetail mb-5 overflow-hidden">
								<li class="mb-2">Product Code: <span id="code"><?=$_GET["pcode"]?></span></li>
								<li class="mb-2">Quantity: <span><?=$row[4]?></span></li>
								<li class="mb-2">Shipping tax: <span>4$</span></li>
								<li class="mb-2">Category: <span><?=$row[3]?></span></li>
							</ul>
						<section class="container-fluid bg">
                        <section class="row justify-content-center">
                            <section class="col-sm-12 col-md-4 mt-5">
								<button onclick="window.location='changeprice.php?pcode=<?=$store?>'" class="btn btnTheme btnShop fwEbold text-white"><a>Change <br> price</a></button>
                            </section>
                            <section class="col-sm-12 col-md-4 mt-5">
								<button id="bt_delete" style="display: inline-block;" class="btn btnTheme btnShop fwEbold text-white">Delete <br> product</button>
                            </section>
                            <section class="col-sm-12 col-md-4 mt-5">
								<button onclick="window.location='changeimage.php?pcode=<?=$store?>'"  class="btn btnTheme btnShop fwEbold text-white ">Change<br> image</button>
                            </section>
                            </section>
                        </section>
                     </section>
